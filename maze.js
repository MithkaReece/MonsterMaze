...
class Maze{
  ...
  checkCell(dir,cell,minh,minv,maxh,maxv){
      let x = cell.getX();
      let y = cell.getY();
      let neighbour;
      if(dir == 0){
...
function wallToTree(){
  for(let i=0;i<walls.length;i++){
    let current = walls[i];
    quad.insert(current.pos.x,current.pos.y,current.l,current.w);
  }
...
class QuadTree{
  ...
  insert(x,y,width,height){
    ...
    for(let i=0;i<this.quads.length;i++){
      ...
      if(current.withinBounds(x,y,width,height)){
        current.insert(x,y,width,height);
        return;
      }
    }
  ...
  pBounds(x,y){
    ...
    //console.log(x,y,this.boundary.x,this.boundary.y,this.boundary.w,this.boundary.h)
    return answer
  }
  ...
  retrieve(){
    ...
    for(let i=0;i<this.quads.length;i++){
      array.push(this.quads[i].retrieve());
    }
    return array;
  }
